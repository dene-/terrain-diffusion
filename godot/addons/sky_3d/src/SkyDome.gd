...
const WORLD_CLOUD_SHADER: String = "res://addons/sky_3d/shaders/WorldClouds.gdshader"
...
const CLOUD_UPDATE_INTERVAL: float = 0.1

var is_scene_built: bool = false
var fog_mesh: MeshInstance3D
var world_cloud_mesh: MeshInstance3D
var sky_material: ShaderMaterial
var cumulus_material: Material
var fog_material: Material
var world_cloud_material: ShaderMaterial
var _cloud_update_accumulator: float = 0.0
...
func _build_scene() -> void:
	...
	# Cumulus belongs to a depth-aware physical altitude shell. The original
	# sky shader remains responsible for cirrus, stars, and atmosphere only.
	world_cloud_mesh = MeshInstance3D.new()
	world_cloud_mesh.name = "_WorldCloudVolume"
	var cloud_screen_quad := QuadMesh.new()
	cloud_screen_quad.size = Vector2(2.0, 2.0)
	world_cloud_mesh.mesh = cloud_screen_quad
	world_cloud_material = ShaderMaterial.new()
	world_cloud_material.shader = load(WORLD_CLOUD_SHADER)
	world_cloud_material.render_priority = fog_render_priority - 1
	world_cloud_mesh.material_override = world_cloud_material
	world_cloud_mesh.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	world_cloud_mesh.custom_aabb = AABB(
		Vector3(-1e31, -1e31, -1e31), Vector3(2e31, 2e31, 2e31))
	add_child(world_cloud_mesh)
	is_scene_built = true
...
func process_tick(delta: float) -> void:
	if not (cirrus_visible or cumulus_visible):
		return
	_cloud_update_accumulator += delta
	if _cloud_update_accumulator < CLOUD_UPDATE_INTERVAL:
		return
	var elapsed: float = _cloud_update_accumulator
	_cloud_update_accumulator = 0.0
	_animation_time += elapsed
	var position_delta: Vector2 = _cloud_velocity * elapsed
	sky_material.set_shader_parameter("animation_time", _animation_time)
	if cumulus_visible:
		_cumulus_position += position_delta
		sky_material.set_shader_parameter("cumulus_position", _cumulus_position)
		world_cloud_material.set_shader_parameter("wind_offset", _cumulus_position)
	if cirrus_visible:
...
func _update_sun_coords() -> void:
	...
	world_cloud_material.set_shader_parameter("sun_direction", _sun_transform.origin)
	if _sun_light_node:
...
@export_range(0.0, 100000.0) var fog_start: float = 0.0 :
	set(value):
		fog_start = value
		if is_scene_built:
...
@export_range(0.0, 100000.0) var fog_end: float = 6000.0 :
	set(value):
		fog_end = value
		if is_scene_built:
...
## Exponential reduction of air density with physical altitude in inverse metres.
## 0.000118 corresponds to an approximately 8.5 km atmospheric scale height.
@export_range(0.0, 0.001, 0.000001, "or_greater") var fog_height_density: float = 0.000118 :
	set(value):
		fog_height_density = value
		if is_scene_built:
			fog_material.set_shader_parameter("fog_height_density", fog_height_density)
...
@export_flags_3d_render var fog_layers: int = 524288 :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_mesh.layers = fog_layers
...
@export var clouds_night_color := Color(0.090196, 0.094118, 0.129412, 1.0) :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_material.set_shader_parameter("night_color", clouds_night_color)
...
@export var cumulus_visible: bool = true :
	set(value):
		...
		if is_scene_built:
			# Never draw cumulus at infinite sky distance. The world-space
			# volume below owns their depth and parallax.
			sky_material.set_shader_parameter("cumulus_visible", false)
			world_cloud_mesh.visible = value
			_check_cloud_processing()


## Physical base of the cumulus layer above mean sea level.
@export_range(500.0, 8000.0, 10.0, "or_greater", "suffix:m") var cumulus_base_height: float = 2200.0 :
	set(value):
		cumulus_base_height = value
		if is_scene_built:
			world_cloud_material.set_shader_parameter("cloud_base_meters", cumulus_base_height)


## Physical top of the cumulus layer above mean sea level.
@export_range(1000.0, 12000.0, 10.0, "or_greater", "suffix:m") var cumulus_top_height: float = 4500.0 :
	set(value):
		cumulus_top_height = value
		if is_scene_built:
			world_cloud_material.set_shader_parameter("cloud_top_meters", cumulus_top_height)
...
@export_range(0, 16, 0.005) var cumulus_intensity: float = 0.6 :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_material.set_shader_parameter("cloud_intensity", cumulus_intensity)
...
@export_range(0.0, 1.0, 0.001) var cumulus_coverage: float = 0.55 :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_material.set_shader_parameter("cloud_coverage", cumulus_coverage)
...
@export var cumulus_absorption: float = 2.0 :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_material.set_shader_parameter("cloud_absorption", cumulus_absorption)
...
@export var cumulus_texture: Texture2D = CUMULUS_TEXTURE :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_material.set_shader_parameter("cloud_noise", cumulus_texture)
...
@export var cumulus_size: float = 0.5 :
	set(value):
		...
		if is_scene_built:
			...
			world_cloud_material.set_shader_parameter(
				"cloud_scale_meters", 12000.0 / maxf(cumulus_size, 0.05))
